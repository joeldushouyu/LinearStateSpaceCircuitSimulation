...
```bash
curl -sSL https://install.python-poetry.org | python3 -  # or other methods of your choice
...
```
3. Run main.py  for pure python simulation
...
Metadata_half_bridge_llc_0.004.h5 is the metadata file generated by the python script main.py. 0.004 is the end time of simulation.
...
make run
```
5. CD back to cppReadCircuitDataLibrary folder to run the C++ simulation under (
    Use Poerty environment(or install all the required python package), see the change in the python environment)
```bash

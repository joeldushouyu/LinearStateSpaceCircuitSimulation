...
float * mv_16_row_with_STATE_U_SIZE_col_parallel(float * matrix,
    ...
    for(uint32_t col = 0; col+1 <loop_iter; col+=2 ){
        ...
        // accum_temp = mac_elem_16_accuracy_safe(a2,b2,  aie::mac(accum_temp, a,b ),0,0,0  );
        accum_temp = mac_elem_16_accuracy_safe(a2,b2,  mac_elem_16_accuracy_safe(a,b, accum_temp,0,0,0  ),0,0,0  );
        // accum_temp = mac_elem_16_accuracy_safe(a,b, accum_temp,0,0,0  );
        // accum_temp = aie::mac( accum_temp,a2,b2  );
    }
...
float * mv_16_row_with_STATE_U_SIZE_col_parallel_FULL_UNROLL(float * matrix,
    aie::vector<float, 16> *x_u_cur, aie::accum<accfloat, 16> &accum_temp){
...
void mult_with_C1_DSW_lock_aware(float *C1_DSW_mat, aie::vector<float, 16> *x_u_cur, uint32_t* c1_res_mask, 
    ...
    for( row = 1; row < C1_DSW_ROW_SIZE_DIV_16; row++){
        ...
        C1_DSW_mat = mv_16_row_with_STATE_U_SIZE_col(
            C1_DSW_mat, x_u_cur, C1_DSW_temp
        );
...
template<uint32_t C1_RES_MASK_LEN>
void mult_with_C1_DSW_FULLY_UNROLL(float *C1_DSW_mat, aie::vector<float, 16> *x_u_cur, uint32_t* c1_res_mask, 
    float*out // for debug
){
    ...
    for(uint32_t row = 0; row < C1_DSW_ROW_SIZE_DIV_16; row++){
        ...
        C1_DSW_mat = mv_16_row_with_STATE_U_SIZE_col_parallel_FULL_UNROLL(
            C1_DSW_mat, x_u_cur, C1_DSW_temp
        );
...
template<uint32_t X_NEXT_BUFFER_SIZE>
void mult_with_A_B_To_Vector_Array_FULLY_UNROLL(float *A_B_C_D_mat, aie::vector<float, 16> *x_u_cur,  aie::vector<float, 16> *x_next_res){
    static_assert(X_NEXT_BUFFER_SIZE == STATE_SIZE_CEIL_TO_16);
    constexpr uint32_t loop_iteration = STATE_SIZE_CEIL_TO_16/16;
    AIE_PREPARE_FOR_PIPELINING
    AIE_LOOP_UNROLL_FULL
    for(uint32_t row = 0; row < loop_iteration; row++){
        aie::accum<accfloat, 16> ABtemp = aie::zeros<accfloat, 16>();
        A_B_C_D_mat = mv_16_row_with_STATE_U_SIZE_col_parallel_FULL_UNROLL(
            A_B_C_D_mat, 
            x_u_cur,
            ABtemp
        );
...
template<uint32_t X_NEXT_BUFFER_SIZE>
void mult_with_A_B_To_Array_FULLY_UNROLL(float *A_B_C_D_mat, aie::vector<float, 16> *x_u_cur, float *x_next_res){
    static_assert(X_NEXT_BUFFER_SIZE == STATE_SIZE_CEIL_TO_16);
    constexpr uint32_t loop_iteration = STATE_SIZE_CEIL_TO_16/16;
    AIE_PREPARE_FOR_PIPELINING
    AIE_LOOP_UNROLL_FULL
...
void mult_with_C_D_aligned_nonimpulse_only_FULLY_UNROLL(float *C_D_mat, aie::vector<float, 16> *x_u_cur, float*out){
    ...
    for(uint32_t row = 0; row < num_of_iteration; row++){
        ...
        C_D_mat= mv_16_row_with_STATE_U_SIZE_col_parallel_FULL_UNROLL(
            C_D_mat, 
            x_u_cur,
            C_D_temp
        );
...
void mult_with_C_D_aligned_nonimpulse_and_impulse_FULLY_UNROLL(float *C_D_mat, aie::vector<float, 16> *x_u_cur, float*out){
    ...
    for(uint32_t row = 0; row < num_of_iteration/2; row++){
        ...
        C_D_mat = mv_16_row_with_STATE_U_SIZE_col_parallel_FULL_UNROLL(C_D_mat, x_u_cur, C_D_temp);
...
void mult_with_C_D_aligned_nonimpulse_and_impulse_lock_aware(float *C_D_mat, aie::vector<float, 16> *x_u_cur, float*out,
    ...
    for(uint32_t row = 1; row < num_of_iteration/2; row++){
        ...
        C_D_mat = mv_16_row_with_STATE_U_SIZE_col(C_D_mat, x_u_cur, C_D_temp);

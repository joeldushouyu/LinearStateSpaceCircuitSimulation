...
void iteration(   float* C1_DSW_buffer, float*ABCD_buffer, float*input_buffers, float *output_buffers , std::vector<uint32_t> &switch_diode_state_reference ,
    ...
    for(uint32_t i = 0; i < ITERATION_STEP_NUMBER; i++ ){
        ...
        for(uint32_t k = 0; k < C1_DSW_ROW_SIZE/16; k++ ){
            ...
            uint32_t gt_res =mask_greater_than_zero(&(C1_DSW_mat_res[k*16]));
            uint32_t lt_res = mask_less_than_zero(&(C1_DSW_mat_res[k*16]));

            uint32_t iteration_bit_shift = 5 *k;  // iteration0: process diode4 - diode 8
                                                  // iteration1: process diode1 - diode 3
